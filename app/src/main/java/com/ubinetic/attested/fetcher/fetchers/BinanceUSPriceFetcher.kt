...
class BinanceUSPriceFetcher : PriceFetcher {
    val PRECISION = BigDecimal.valueOf(Math.pow(10.0,6.0))
    val SYMBOL = "XTZUSD"

    override fun fetchPrice(success: (Price) -> Unit, error: (Exception) -> Unit) {
        val url =
            URL("https://api.binance.us/api/v3/klines?symbol=${SYMBOL}&interval=15m&startTime=${Utils.getPreviousEpochStart()}&endTime=${Utils.getPreviousEpochEnd()}")
